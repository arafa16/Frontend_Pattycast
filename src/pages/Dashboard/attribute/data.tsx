...
const Data = (props) => {
    const {dataPengajuans, limit, page, allPage, allData, nextPage, prevPage, typePengajuans, changeType, type, loading, changeSearch, search} = props;
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-12 mt-5 box mb-8">
                ...
                <div>
                    <div
                        className={`flex items-center px-3 py-2 font-medium rounded-md cursor-pointer ${type === 0 ? "text-white bg-primary" : "hover:text-white hover:bg-primary" } dark:bg-darkmode-800`}
                    ...
                    {typePengajuans.map((typePengajuan, index)=>(
                        <div
                            onClick={()=>changeType(typePengajuan.code)}
                            key={index}
                            className={`flex items-center px-3 py-2 mt-2 cursor-pointer rounded-md  ${type === typePengajuan.code ? "text-white bg-primary" : "hover:text-white hover:bg-primary" } dark:bg-darkmode-800`}
                        >
                            <Lucide icon="Star" className="w-4 h-4 mr-2" /> 
                            {typePengajuan.name} 
                            {loading ? <LoadingIcon icon="ball-triangle" color="white" className="w-3 h-5 mx-5" /> : ''}
                ...
                <div className="overflow-x-auto sm:overflow-x-visible">
                    ...
                    <div 
                        key={index} 
                        className="intro-y"
                        onClick={()=>navigate(`/formUpdate/${data.uuid}`)}
                    >
                        <div
                        className={clsx([
