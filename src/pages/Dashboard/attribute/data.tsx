                ...
                <div>
                    ...
                    {dataStatus.map((data : any, index : any)=>(
                        <div
                            ...
                            key={index}
                            className={`flex items-center px-3 py-2 mt-2 cursor-pointer rounded-md  ${status === data.code ? "text-white bg-primary" : "hover:text-white hover:bg-primary" } dark:bg-darkmode-800`}
                        >
                            <Lucide icon="Star" className="w-4 h-4 mr-2" /> 
                            {data.name}
                            {loading ? <LoadingIcon icon="ball-triangle" color="white" className="w-3 h-5 mx-5" /> : ''}
